...
class PathPlanningEnv(gym.Env):
    ...
    def __init__(self, building_generator, start, goal, x_range, y_range, z_range, step_size=100):
...
def smooth_path_ma(path, window_size=5):
    """
    path: [(x0,y0,z0), (x1,y1,z1), ...]
    window_size: 홀수 권장 (ex. 3,5,7)
    """
    import numpy as np
    ...
    for i in range(n):
        # 시작점, 끝점은 원본 그대로
        if i == 0 or i == n - 1:
            smoothed.append(tuple(pts[i]))
        else:
            start = max(0, i - half)
            end   = min(n, i + half + 1)
            window_avg = pts[start:end].mean(axis=0)
            smoothed.append(tuple(window_avg))
...
if __name__ == '__main__':
    ...
    model_path = '3DSSP_RL_step100.zip'
    ...
    smoothed = smooth_path_ma(path, window_size=11)
    end_time = time.time()

    print(f"총 이동 거리: {compute_total_distance(smoothed):.2f}")
    print(f"총 소요 시간: {end_time - start_time:.2f}초")
    visualize_path(test_env.obstacles, smoothed, start, goal)
